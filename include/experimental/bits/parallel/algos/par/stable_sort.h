...
namespace parallel {
  ...
  template<class RandomAccessIterator>
    void parallel_execution_policy::stable_sort(RandomAccessIterator first, RandomAccessIterator last) const 
  {
    return this->stable_sort(first, last, 
                             std::less<typename iterator_traits<RandomAccessIterator>::value_type>());
  }

  template<class RandomAccessIterator, class Compare>
    void parallel_execution_policy::stable_sort(RandomAccessIterator first, RandomAccessIterator last, 
                                                Compare comp) const 
  {
    // 2 stage sort:
    ...
    // stage 1
    detail::diffract(first, last, std::stable_sort<RandomAccessIterator, Compare>, comp);
